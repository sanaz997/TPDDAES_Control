function [z,g_mz,gg,d_vec]=FM_computation_UPO(rddae,M,varargin)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% disp('here')
M_=M+1; 
num=nargin-2;
...
if isfield(opts,'gain')
  ...
  %%% If no gain is given, the spectral radius of the open-loop system will
  %%% be calcualted.
    gain1=zeros(r,p);
    gain.K=gain1;
    gain.type='TC';
...
z=d3(ind(1));
%%
%%% Required adjustments for the autonomous part:
...
tolerance=1e-8;
d4 =d3(abs(abs(d3) - 1) > tolerance);
V2=V2(:,abs(abs(d3) - 1) > tolerance);
W2=W2(:,abs(abs(d3) - 1) > tolerance);
[g_val,ind]=sort(abs(d4),'descend');
z=d4(ind(1));
  gg=d4(ind(1:end));
RF=V2(:,ind(1));
LF=W2(:,ind(1));
